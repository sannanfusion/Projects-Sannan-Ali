...
import { Dumbbell, Utensils, Wine, Coffee, Clock, Car, Plane, MapPin, Waves, Users, Music, BookOpen, Heart } from "lucide-react";
...
export default function Amenities() {
  ...
  const getIcon = (categoryName: string, index: number) => {
    const icons = {
      wellness: [<Spa key={0} />, <Dumbbell key={1} />, <Waves key={2} />, <Coffee key={3} />], // replaced Yoga with Coffee
      dining: [<Utensils key={0} />, <Coffee key={1} />, <Wine key={2} />, <Clock key={3} />],
      services: [<Clock key={0} />, <Plane key={1} />, <Car key={2} />, <MapPin key={3} />], // replaced CarIcon with Car
